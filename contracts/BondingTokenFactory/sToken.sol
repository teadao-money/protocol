...
abstract contract BEP20 is IBEP20 {
   ...
   * construction.
   */
    function initializeBEP20(string memory name_, string memory symbol_, uint8 decimals_) internal {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
    }
...
abstract contract BEP20Permit is BEP20, IBEP2612Permit {
    ...
    function initializeBEP20Permit() internal {
...
contract Ownable is IOwnable {
    ...
    function initalizeOwnable(address __owner) internal {
        _owner = __owner;
...
contract sTokenImplement is Singleton, BEP20Permit, Ownable {
    ...
    function initialize(address stakingContract_, address __owner, string calldata name, uint8 dec, uint256 totalSup) external returns (bool) {
        ...
        INITIAL_FRAGMENTS_SUPPLY = totalSup * 10 ** dec;
        TOTAL_GONS = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
        MAX_SUPPLY = ~uint128(0);
        require(manager() == address(0));
        initializeBEP20(string(abi.encodePacked("Staking ", name)), string(abi.encodePacked('s', name)), dec);
        initializeBEP20Permit();
        initalizeOwnable(__owner);
