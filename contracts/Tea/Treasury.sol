...
contract TeaTreasury is Ownable {
    ...
    enum MANAGING {RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STEA}
    ...
    address public sTEA;
    uint public sTEAQueue; // Delays change to sTEA address
    ...
    //        address _TEADAI,
        ...
        //       isLiquidityToken[ _TEADAI ] = true;
        //       liquidityTokens.push( _TEADAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for TEA
        @param _amount uint
    ...
    function deposit(uint _amount, address _token, uint _profit) external returns (uint send_) {
        ...
        // mint TEA needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(TEA).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn TEA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint _amount, address _token) external {
        ...
        uint value = valueOf(_token, _amount);
        ITEAERC20(TEA).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint _amount, address _token) external {
        ...
        uint maximumDebt = IERC20(sTEA).balanceOf(msg.sender);
        // Can only borrow against sTEA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TEA
        @param _amount uint
     */
    function repayDebtWithTEA(uint _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        ITEAERC20(TEA).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, TEA, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint _amount) external {
        ...
        IERC20Mintable(TEA).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint) {
        return totalReserves.sub(IERC20(TEA).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns TEA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(address _token, uint _amount) public view returns (uint value_) {
        if (isReserveToken[_token]) {
            // convert amount to match TEA decimals
            value_ = _amount.mul(10 ** IERC20(TEA).decimals()).div(10 ** IERC20(_token).decimals());
    ...
    function queue(MANAGING _managing, address _address) external onlyManager() returns (bool) {
        ...
        } else if (_managing == MANAGING.STEA) {// 9
            sTEAQueue = block.number.add(blocksNeededForQueue);
        } else return false;
