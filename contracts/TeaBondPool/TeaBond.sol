...
contract Ownable {
     ...
     */
    function initOwnable(address _setOwner) internal {
        _owner = _setOwner;
...
interface IPriceData {
    ...
    function getPrice(address _principle) external view returns (ResponsePriceData memory);
}
...
contract TeaBond is Pausable {
     ...
     *  @return uint
     */
    function deposit(address _depositor, uint256 _amount) external payable whenNotPaused returns (uint){

        require(_depositor != address(0), "Invalid address");
        //Calculate lp token to tea
        uint value = uint256(IPriceData(calculator).getPrice(principle).rate).mul(_amount).div(10 ** 18);
        //Apply discount
    ...
    function redeem(address _recipient) external returns (uint) {
        ...
        if (percentVested >= 1000000000) {// if fully vested
            ...
            // emit bond data
            return stakeOrSend(_recipient, info.payout);
            // send payout
