...
const BondingCalculator = artifacts.require(
  "contracts/poolBonding/TeaPriceFeed.sol:PriceFeed"
);
...
const TeaPriceFeedNFT = artifacts.require(
  "contracts/poolBonding/TeaPriceFeedNFT.sol:TeaPriceFeedNFT"
);
const TeaNFT721 = artifacts.require("TeaNFT721");
const TeaBondNFT721 = artifacts.require("TeaBondNFT721");
const TeaTreasury = artifacts.require(
  "contracts/poolBonding/TeaTreasury.sol:TeaTreasury"
);
const TeaStaking = artifacts.require(
  "contracts/poolBonding/TeaStaking.sol:TeaStaking"
);
const STea = artifacts.require("sTea");
const fs = require("fs");
const HDWalletProvider = require("@truffle/hdwallet-provider");
const mnemonic = fs.readFileSync("./.secret").toString().trim();
const listWallets = new HDWalletProvider(
  mnemonic,
  "https://kovan.infura.io/v3/8d62942fd62641a7ab758673105b6df3"
).wallets;
...
contract("TeaToken\n", function (accounts) {
  ...
  describe("Test full flow\n", function () {
    it("Test", async function () {
      pancakeFactory = await PancakeFactory.new(governance, {
        from: governance,
      });
      let CHAINID = pancakeFactory.constructor.network_id;
      console.log(CHAINID == 97);
      console.log(
        "\n1. create pancakeFactory bsc contract: https://testnet.bscscan.com/address/" +
          pancakeFactory.address
      );

      wETH9 = await WETH9.new({ from: governance });
      console.log(
        "\n2. create WETH9 bsc token contract: https://testnet.bscscan.com/address/" +
          wETH9.address
      );
      ...
      console.log(
        "\n2. create daiToken bsc token contract: https://testnet.bscscan.com/address/" +
          daiToken.address
      );

      let teaToken = await TeaToken.new(500, account4);
      console.log(
        "\n4. create TeaToken contract: https://testnet.bscscan.com/address/" +
          teaToken.address
      );

      transaction = await pancakeFactory.createPair(
        teaToken.address,
        wETH9.address
      );
      console.log(
        "\n8.  create pair transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );
      ...
      console.log(
        "\n9. TeaToken setAddressPair transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );

      let calculator = await BondingCalculator.new();
      console.log(
        "\n13. BondingCalculator transaction: https://testnet.bscscan.com/address/" +
          calculator.address
      );

      let teaTreasury;
      if (CHAINID === 97) {
        teaTreasury = await TeaTreasury.at(
          "0x297c0337f8eA3e9d3E3cb572F94A74b61CAd50Be"
        );
      } else {
        teaTreasury = await TeaTreasury.new(teaToken.address, pair.address);
      }
      console.log(
        "\n14. teaTreasury address: https://testnet.bscscan.com/address/" +
          teaTreasury.address
      );

      transaction = await teaToken.addMinter(teaTreasury.address);
      console.log(
        "\n15. TeaToken.addMinter transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );

      let sTea = await STea.new();
      console.log(
        "\n16. STea address: https://testnet.bscscan.com/address/" +
          sTea.address
      );
      ...
      let teaStaking = await TeaStaking.new(
        teaToken.address,
        sTea.address,
        epochLengthRebase,
        0,
      ...
      console.log(
        "\n17. TeaStaking address: https://testnet.bscscan.com/address/" +
          teaStaking.address
      );

      transaction = await sTea.initialize(teaStaking.address);
      console.log(
        "\n18. STea initialize transaction: https://testnet.bscscan.com/address/" +
          transaction.tx
      );
      ...
      let teaPoolFactory = await TeaPoolFactory.new(
        teaTreasury.address,
        teaToken.address,
        calculator.address,
        teaStaking.address
      ...
      transaction = await teaTreasury.setIsRewardManager(
        teaStaking.address,
        true
      );
      console.log(
        "\n21. teaTreasury setDepositor  transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );
      ...
      console.log(
        `19. transaction teaPoolFactory.setTokenBond ${transaction.tx}`
      );

      transaction = await teaTreasury.addIsSetter(teaPoolFactory.address, true);
      console.log(
        "\n20. transaction teaTreasury addIsSetter transaction: https://testnet.bscscan.com/address/" +
          transaction.tx
      );

      transaction = await teaPoolFactory.createPool(
        governance,
        daiToken.address,
        governance,
        controlVariable,
      ...
      console.log(
        `20. transaction teaPoolFactory.createPool ${transaction.tx}`
      );
      ...
      transaction = await daiToken.approve(
        poolNew.address,
        web3.utils.toWei("10000000000000000")
      );
      console.log(
        "\n23. daiToken approve transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );

      transaction = await poolNew.addLiquidity(web3.utils.toWei("1"));
      console.log(
        "\n24. transaction daiBond.deposit transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );

      transaction = await poolNew.redeem();
      console.log(
        "\n25. poolNew redeem transaction: https://testnet.bscscan.com/tx/" +
          transaction.tx
      );
      ...
      let teaBondNFT721 = await TeaBondNFT721.new(
        teaToken.address,
        nft721.address,
        teaPriceFeedNFT.address,
        poolNew.address,
        governance
      );
      console.log(`29. teaBondNFT721 ${teaBondNFT721.address}`);

      transaction = await teaBondNFT721.initializeBondTerms(
        controlVariable,
        vestingTerm,
        maxPayout,
        fee
      ...
      console.log(
        `30. transaction teaBondNFT721.initializeBondTerms ${transaction.tx}`
      );
      ...
      console.log(
        `32. transaction teaPriceFeedNFT.setSupportNFT ${transaction.tx}`
      );
      ...
      transaction = await teaPriceFeedNFT.fulfillPrice(reqId, [
        web3.utils.toWei("0.1"),
      ]);
      console.log(
        `36 transaction teaPriceFeedNFT.fulfillPrice ${transaction.tx}`
      );
