...
async function main() {
  ...
  const Treasury = await ethers.getContractFactory("contracts/TeaBonding/Treasury.sol:TeaTreasury");
  const treasury = await Treasury.deploy(tea.address, dai.address, dai.address, 0);

  // Deploy bonding calc
  const TeaBondingCalculator = await ethers.getContractFactory("TeaBondingCalculator");
  const olympusBondingCalculator = await TeaBondingCalculator.deploy(tea.address);

  // Deploy staking distributor
  const Distributor = await ethers.getContractFactory("Distributor");
  const distributor = await Distributor.deploy(treasury.address, tea.address, epochLengthInBlocks, firstEpochBlock);
  ...
  // Deploy Staking
  const Staking = await ethers.getContractFactory("contracts/TeaBonding/Staking.sol:TeaStaking");
  const staking = await Staking.deploy(tea.address, sTEA.address, epochLengthInBlocks, firstEpochNumber, firstEpochBlock);

  // Deploy staking warmpup
  const StakingWarmpup = await ethers.getContractFactory("StakingWarmup");
  const stakingWarmup = await StakingWarmpup.deploy(staking.address, sTEA.address);

  // Deploy staking helper
  const StakingHelper = await ethers.getContractFactory("StakingHelper");
  const stakingHelper = await StakingHelper.deploy(staking.address, tea.address);

  // Deploy DAI bond
  //@dev changed function call to Treasury of 'valueOf' to 'valueOfToken' in BondDepository due to change in Treausry contract
  const DAIBond = await ethers.getContractFactory("TeaBondDepository");
  const daiBond = await DAIBond.deploy(tea.address, dai.address, treasury.address, MockDAO.address, zeroAddress);
  ...
  const pancakeRouter = await PancakeRouter.attach("0x3684bbC718cD82598f5B7993d6FF42B6cc4B15c8");


  await tea.approve(pancakeRouter.address, web3.utils.toWei("100000", "ether"));
  await dai.approve(pancakeRouter.address, web3.utils.toWei("20000000000", "ether"));
  ...
  await pancakeRouter.addLiquidity(tea.address, dai.address, web3.utils.toWei("8000", "gwei"), web3.utils.toWei("2400000", "ether"), 0, 0, deployer.address, 10000000000000);
